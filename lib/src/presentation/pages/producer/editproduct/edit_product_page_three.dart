...
class EditProductPageThree extends ConsumerStatefulWidget {
  const EditProductPageThree({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _EditProductPageThreeState();
}

class _EditProductPageThreeState extends ConsumerState<EditProductPageThree> {
  ...
  Widget build(BuildContext context) {
    ...
    final finalImageList = images
        .where((item) => item.uuid != ImageState.deleted.label)
        .toList();
              ...
              Column(
                ...
                children: [
                  if (createProductState.discountedPrice != null)
                    Text(
                      '${(createProductState.discountedPrice ?? 0).toLocalNumber(local)}  افغانی',
                      maxLines: 1,
